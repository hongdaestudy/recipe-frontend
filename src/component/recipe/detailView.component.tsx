    ...
    if (i % sliderItems === 0) {
      items.push(
        <Grid
          ...
          spacing={2}
          key={i.toString()}
          sx={{ width: '700px', height: 'auto', margin: '0 auto' }}
        >
...
export default function DetailView() {
  ...
  const handleRecipeStepViewChange = (
    event: React.MouseEvent<HTMLElement, MouseEvent>,
    recipeStepView: string,
  ) => setRecipeStepView(recipeStepView);
  ...
  return (
    <>
      ...
      <ContentWrapper>
        ...
        <SummaryInfo>
          <SummaryInfoSpan src="https://recipe1.ezmember.co.kr/img/mobile/icon_man.png">
            {recipe.servingCount}
          </SummaryInfoSpan>
          <SummaryInfoSpan src="https://recipe1.ezmember.co.kr/img/mobile/icon_time2.png">
            {recipe.cookingTime}
          </SummaryInfoSpan>
          <SummaryInfoSpan src="https://recipe1.ezmember.co.kr/img/mobile/icon_star.png">
      ...
      <ContentWrapper>
        ...
        <div>
          {recipe.ingredientGroups?.map(
            (ingredientGroup, index, ingredientGroups) => (
              <IngredientGroupDiv key={index} length={ingredientGroups.length}>
                ...
                <ul>
                  {ingredientGroup.ingredients.map((ingredient, i) => (
                    <IngredientLi key={i} length={ingredientGroups.length}>
        ...
        {recipe.recipeSteps?.map((recipeStep, index) => (
          <Grid key={index} container spacing={2}>
            ...
            <Grid
              item
              xs={
                recipeStepView === 'image'
                  ? 4
                  : recipeStepView === 'bigImage'
                  ? 12
                  : 0
              }
              sx={{ textAlign: 'center' }}
      ...
      <ContentWrapper>
        ...
        <DateInfoDiv>
          ...
          <DateNotice>
            저작자의 사전 동의 없이 이미지 및 문구의 무단 도용 및 복제를
            금합니다.
          </DateNotice>
        </DateInfoDiv>
      </ContentWrapper>
      ...
      <ContentWrapper>
        <h2>
          댓글{' '}
          <span style={{ color: '#74b243' }}>{recipe.comments?.length}</span>
        </h2>
      ...
      <ContentWrapper>
        <img
          src="https://recipe1.ezmember.co.kr/img/mobile/icon_tag.png"
          alt=""
          style={{ verticalAlign: 'bottom' }}
        />
        {recipe.tags?.map(item => (
          <Button key={item.tagName} variant="outlined" color="success">
            #{item.tagName}
          </Button>
        ))}
      </ContentWrapper>
...
const _IngredientGroupDiv = ({
  length,
  children,
  className,
}: IngredientGroupDivProps) => <div className={className}>{children}</div>;
const IngredientGroupDiv = styled(_IngredientGroupDiv)`
  width: ${(props: IngredientGroupDivProps) =>
    props.length > 1 ? '50%' : '100%'};
  display: inline-block;
  vertical-align: top;
`;
...
const DateNotice = styled.span`
  display: inline-block;
  background: url('https://recipe1.ezmember.co.kr/img/icon_noti.png') left top
    no-repeat;
